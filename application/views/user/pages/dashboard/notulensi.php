			...
			<?php foreach ($notulensi as $row): ?>
				<div class="col-lg-4 col-md-6">
					<article class="card border-0 rounded-0 mb-4">
						<img src="<?= base_url('uploads/notulensi/').$row['thumbnail'] ?>" alt="<?=$row['thumbnail']?>" class="img-fluid">
						<div class="mt-3 px-4 py-3 ">
							...
							<a href="<?= base_url('user/dashboard/detailNotulensi/').$row['id'] ?>"><h4 class="mb-3 font-secondary"><?= $row['judul'] ?></h4></a>
							<?php 
							...
							if ($jml_karakter>50) {
							 ...
							 ?>
							<p class="mb-4"><?= $isi ?>...</p>
							<a href="<?= base_url('user/dashboard/detailNotulensi/').$row['id'] ?>" class="text-color mb-3 d-block"><i class="ti-minus mr-2"></i> Read More</a>
							<!-- stretched-link -->
						</div>
					</article>
				</div>
